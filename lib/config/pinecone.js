...
// --- Workflow examples index (Vyrade's workflow-intelligence moat) ---
// Semantic search returns metadata.mysql_id; the full WORKFLOW_JSON is then
// hydrated from MySQL (see workflowExampleRepository). Isolated and optional —
// when unset, n8n generation degrades to node + tool retrieval.
//
// NOTE: the deployed .env spells these "EXMAPLE"; both spellings are accepted
// so the feature works with existing config and with the corrected name.
export const PINECONE_WORKFLOW_INDEX =
  process.env.PINECONE_WORKFLOW_EXAMPLE_INDEX || process.env.PINECONE_WORKFLOW_EXMAPLE_INDEX;
export const PINECONE_WORKFLOW_NAMESPACE =
  process.env.PINECONE_WORKFLOW_EXAMPLE_NAMESPACE || process.env.PINECONE_WORKFLOW_EXMAPLE_NAMESPACE || undefined;
export const WORKFLOW_EMBEDDING_MODEL =
  process.env.PINECONE_WORKFLOW_EMBEDDING_MODEL || EMBEDDING_MODEL;
const workflowApiKey = () =>
  process.env.PINECONE_WORKFLOW_EXAMPLE_API_KEY || process.env.PINECONE_WORKFLOW_EXMAPLE_API_KEY;
...
// --- Workflow examples ---
export function isWorkflowIndexConfigured() {
  return Boolean(workflowApiKey() && PINECONE_WORKFLOW_INDEX);
}

export function getPineconeWorkflowIndex() {
  if (!isWorkflowIndexConfigured()) return null;
  if (!_workflowIndex) {
    const pc = new Pinecone({ apiKey: workflowApiKey() });
    const idx = pc.index(PINECONE_WORKFLOW_INDEX);
    _workflowIndex = PINECONE_WORKFLOW_NAMESPACE ? idx.namespace(PINECONE_WORKFLOW_NAMESPACE) : idx;
  }
  return _workflowIndex;
}
