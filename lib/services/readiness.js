 ...
 * flagged as unknown, so nothing can be counted as blocking without also being
 * askable. Items are de-duped by path and ordered blocking-first.
 */
...
export function checkReadiness(bp) {
  ...
  const blocking = q.filter((g) => g.blocking).map((g) => g.path);
