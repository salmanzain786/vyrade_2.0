...
// Keys whose value is never a credential, so they survive verbatim.
const NEVER_SECRET_KEYS = new Set(['command', 'type', 'transport', 'cwd', 'description', 'name']);

// Guard against pathological nesting in third-party catalog data.
const MAX_DEPTH = 12;

/** One string value: redact if its KEY looks secret-like or its VALUE looks like a live token. */
function sanitizeString(key, value, serverName) {
  if (NEVER_SECRET_KEYS.has(key)) return value;
  if (isPlaceholder(value)) return value;                 // already safe
  if (SECRET_NAME_RE.test(key) || SECRET_VALUE_RE.test(value)) {
    return placeholderFor(serverName, key || 'SECRET');
  }
  return value;
}

/**
 * Recursively sanitize any value inside a server config — objects, arrays, and
 * strings at ANY depth. A partial sanitizer is a security bug: a nested field
 * like `auth: { client_secret: "…" }` sits outside env/headers/args and would
 * otherwise be published verbatim.
 *
 * `parentKey` is carried into arrays so `tokens: ["ghp_…"]` is still judged
 * against its key as well as its value.
 */
function sanitizeNode(node, serverName, parentKey = '', depth = 0) {
  if (depth > MAX_DEPTH) return '${REDACTED}'; // never emit unvetted deep data

  if (Array.isArray(node)) {
    return node.map((v) => sanitizeNode(v, serverName, parentKey, depth + 1));
  }
  ...
  if (node && typeof node === 'object') {
    ...
    for (const [k, v] of Object.entries(node)) {
      if (k === 'env' && v && typeof v === 'object' && !Array.isArray(v)) {
        // Every env value is a placeholder — env is where secrets belong, and
        // the user supplies them from their own .env.
        out[k] = Object.fromEntries(Object.keys(v).map((ek) => [ek, placeholderFor(serverName, ek)]));
      } else if (k === 'args' && Array.isArray(v)) {
        // Positional handling: `-e KEY=VALUE`, `--clientSecret <value>`, bare tokens.
        out[k] = sanitizeArgs(v, serverName);
      } else {
        out[k] = sanitizeNode(v, serverName, k, depth + 1);
      }
    }
    return out;
  }

  if (typeof node === 'string') return sanitizeString(parentKey, node, serverName);
...
export function sanitizeMcpConfig(raw) {
  ...
  if (cfg.mcpServers && typeof cfg.mcpServers === 'object' && !Array.isArray(cfg.mcpServers)) {
    ...
    for (const [serverName, server] of Object.entries(cfg.mcpServers)) {
      servers[serverName] = sanitizeNode(server, serverName, '', 0);
    }
    return JSON.stringify({ ...cfg, mcpServers: servers }, null, 2);
  }

  // A bare server object (no mcpServers wrapper).
  return JSON.stringify(sanitizeNode(cfg, '', '', 0), null, 2);
}
