...
import {
  ...
  getPineconeWorkflowIndex, isWorkflowIndexConfigured,
  EMBEDDING_MODEL, TOOL_EMBEDDING_MODEL, MCP_EMBEDDING_MODEL,
  MAKE_EMBEDDING_MODEL, ZAPIER_EMBEDDING_MODEL, WORKFLOW_EMBEDDING_MODEL,
} from '../config/pinecone.js';
import { getWorkflowExamplesByIds } from './workflowExampleRepository.js';
...
// --- Workflow examples (Vyrade's workflow-intelligence layer) ---

/**
 * Build queries that describe the SHAPE of the automation, not just its parts —
 * we want whole workflows that solved a similar problem end-to-end.
 */
function buildWorkflowExampleQueries(bp) {
  const goal = bp.business_intent?.business_goal || bp.name || 'automation';
  const trigger = bp.trigger?.event || bp.trigger?.trigger_type || 'a trigger';
  const systems = (bp.systems || []).map((s) => s.name).filter(Boolean);
  const queries = [`n8n workflow to ${goal}, triggered by ${trigger}.`];
  if (systems.length) {
    queries.push(`n8n workflow connecting ${systems.join(' and ')}.`);
  }
  const actions = [...new Set((bp.process_steps || []).map((s) => s.action_type))];
  if (actions.length) {
    queries.push(`n8n workflow that performs ${actions.join(', ')}${systems.length ? ` using ${systems[0]}` : ''}.`);
  }
  return queries.slice(0, 4);
}

/**
 * Retrieve complete, real n8n workflows to use as structural templates.
 *
 * Two-stage: Pinecone gives semantic relevance and a `mysql_id`, then the full
 * WORKFLOW_JSON is hydrated from MySQL and compacted to a skeleton. We
 * deliberately over-fetch from Pinecone because the vector index can reference
 * rows this database doesn't have yet — unhydratable matches are skipped, so
 * coverage improves automatically as workflow data is migrated in.
 *
 * Never throws: on any failure the specialist simply proceeds with node + tool
 * knowledge (examples are an enhancement, not a dependency).
 */
export async function retrieveWorkflowExamples(bp, { topK = 12, limit = 3, maxChars = 5000 } = {}) {
  const empty = { examples: [], matchCount: 0, queryCount: 0, available: false };
  if (!isWorkflowIndexConfigured()) return empty;
  const index = getPineconeWorkflowIndex();
  if (!index) return empty;

  try {
    const queries = buildWorkflowExampleQueries(bp);
    const vectors = await embed(queries, WORKFLOW_EMBEDDING_MODEL);

    // Best score wins per workflow, across all queries.
    const scoreById = new Map();
    await Promise.all(vectors.map(async (vector) => {
      const res = await index.query({ topK, vector, includeMetadata: true });
      for (const m of res.matches || []) {
        const id = Number(m.metadata?.mysql_id);
        if (!Number.isFinite(id)) continue;
        if (!scoreById.has(id) || m.score > scoreById.get(id)) scoreById.set(id, m.score);
      }
    }));

    const ranked = [...scoreById.entries()].sort((a, b) => b[1] - a[1]).map(([id]) => id);
    if (ranked.length === 0) return { ...empty, available: true, queryCount: queries.length };

    // Hydrate in relevance order, then keep the first `limit` that survive.
    const hydrated = await getWorkflowExamplesByIds(ranked);
    const examples = [];
    let total = 0;
    for (const ex of hydrated) {
      const size = ex.skeleton.length + (ex.description?.length || 0);
      if (examples.length >= limit || total + size > maxChars) break;
      examples.push(ex);
      total += size;
    }

    return { examples, matchCount: ranked.length, queryCount: queries.length, available: true };
  } catch (err) {
    console.error('[retrieval] workflow examples unavailable:', err.message);
...
// Make's index stores APP+ACTION rows (a real module catalog).
...
// Zapier's index is an APP catalog only (no actions/triggers), with plan flags.
// We therefore recommend the app + its caveats and never invent action names.
const truthy = (v) => v === true || v === 'true' || v === 1 || v === '1';
function toZapierRecord(id, score, m = {}) {
  return {
    id,
    score,
    app: m.name || '',
    appSlug: m.slug || '',
    url: m.external_url || '',
    premium: truthy(m.is_premium),
    beta: truthy(m.is_beta),
    upcoming: truthy(m.is_upcoming),
    tier: m.partner_tier || '',
...
/**
 * Retrieve platform records (Make modules / Zapier apps) per Blueprint system
 * from an isolated index. Callers supply the query strings because each index
 * has its own shape and matches best with a different phrasing.
 * Returns { perSystem, all, matchCount, queryCount, available }.
 */
async function retrievePlatformModules({ index, model, toRecord, labels, queries, topK }) {
  const empty = { perSystem: {}, all: [], matchCount: 0, queryCount: 0, available: false };
  if (!index || queries.length === 0) return empty;
...
const EMPTY_MODULES = { perSystem: {}, all: [], matchCount: 0, queryCount: 0, available: false };
const systemNames = (bp) => (bp.systems || []).map((s) => s.name).filter(Boolean);

export async function retrieveMakeModules(bp, { topK = 4 } = {}) {
  if (!isMakeIndexConfigured()) return EMPTY_MODULES;
  const systems = systemNames(bp);
  const goal = bp.business_intent?.business_goal || bp.name || 'automation';
  return retrievePlatformModules({
    index: getPineconeMakeIndex(),
    model: MAKE_EMBEDDING_MODEL,
    toRecord: toMakeRecord,
    labels: [...systems, '__goal__'],
    queries: [
      ...systems.map((n) => `Make.com app modules/actions for ${n}: trigger, create, update, send.`),
      `Make.com modules to ${goal}.`,
    ],
    topK,
  });
}

export async function retrieveZapierModules(bp, { topK = 3 } = {}) {
  if (!isZapierIndexConfigured()) return EMPTY_MODULES;
  const systems = systemNames(bp);
  if (systems.length === 0) return EMPTY_MODULES;
  // The Zapier index holds short app records (name/slug/links), so a bare app
  // name matches far better than a verbose sentence — a descriptive query
  // returns unrelated Zapier-branded apps instead of the real integration.
  return retrievePlatformModules({
    index: getPineconeZapierIndex(),
    model: ZAPIER_EMBEDDING_MODEL,
    toRecord: toZapierRecord,
    labels: systems,
    queries: systems.map((n) => n),
    topK,
  });
}
