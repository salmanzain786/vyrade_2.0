import { v4 as uuidv4 } from 'uuid';
import { and, desc, eq } from 'drizzle-orm';
import { db } from '../config/db.js';
import {
  automationBlueprints,
  automationBlueprintVersions,
  blueprintWorkflows,
} from '../db/schema.js';

export class VersionConflictError extends Error {
  constructor(expected, actual) {
    super(`Version conflict: expected ${expected}, current is ${actual}`);
    this.name = 'VersionConflictError';
    this.statusCode = 409;
  }
}

// mysql2 usually hands back JSON columns already parsed; be defensive either way.
const asJson = (v) => (typeof v === 'string' ? JSON.parse(v) : v);

export async function createBlueprint({ sessionId, userId = null, content, readiness, createdBy = 'ai', sourceTurnId = null }) {
  const blueprintId = uuidv4();
  const versionId = uuidv4();
  const status = readiness.status;

  await db.transaction(async (tx) => {
    await tx.insert(automationBlueprints).values({
      id: blueprintId,
      sessionId,
      userId,
      currentVersion: 1,
      status,
    });

    await tx.insert(automationBlueprintVersions).values({
      id: versionId,
      blueprintId,
      version: 1,
      schemaVersion: '1.0',
      blueprintJson: content,
      readinessJson: readiness,
      changeReason: 'Initial Blueprint from first clarified turn',
      sourceTurnId,
      createdBy,
    });
  });

  return { blueprintId, version: 1, status };
}

export async function patchBlueprint({
  blueprintId, expectedVersion, content, readiness,
  changeReason, sourceTurnId, createdBy = 'ai',
}) {
  // Throwing inside the callback rolls the transaction back.
  return db.transaction(async (tx) => {
    const rows = await tx
      .select({ currentVersion: automationBlueprints.currentVersion })
      .from(automationBlueprints)
      .where(eq(automationBlueprints.id, blueprintId))
      .for('update');

    if (rows.length === 0) throw new Error(`Blueprint not found: ${blueprintId}`);

    const currentVersion = rows[0].currentVersion;
    if (currentVersion !== expectedVersion) {
      throw new VersionConflictError(expectedVersion, currentVersion);
    }

    const newVersion = currentVersion + 1;

    await tx.insert(automationBlueprintVersions).values({
      id: uuidv4(),
      blueprintId,
      version: newVersion,
      schemaVersion: '1.0',
      blueprintJson: content,
      readinessJson: readiness,
      changeReason: changeReason || null,
      sourceTurnId: sourceTurnId || null,
      createdBy,
    });

    await tx
      .update(automationBlueprints)
      .set({ currentVersion: newVersion, status: readiness.status })
      .where(eq(automationBlueprints.id, blueprintId));

    return { blueprintId, version: newVersion, status: readiness.status };
  });
}

/**
 * The user_id that owns a blueprint, or null if the blueprint doesn't exist.
 * Used to authorize mutations/reads on a blueprint by its id.
 */
export async function getBlueprintOwnerId(blueprintId) {
  const [row] = await db
    .select({ userId: automationBlueprints.userId })
    .from(automationBlueprints)
    .where(eq(automationBlueprints.id, blueprintId))
    .limit(1);
  return row ? row.userId : undefined; // undefined = not found; null = unowned
}

/** The session id a blueprint belongs to (for pulling conversation context). */
export async function getBlueprintSessionId(blueprintId) {
  const [row] = await db
    .select({ sessionId: automationBlueprints.sessionId })
    .from(automationBlueprints)
    .where(eq(automationBlueprints.id, blueprintId))
    .limit(1);
  return row ? row.sessionId : null;
}

async function findParent(blueprintId) {
  const [parent] = await db
    .select()
    .from(automationBlueprints)
    .where(eq(automationBlueprints.id, blueprintId))
    .limit(1);
  return parent || null;
}

async function findVersion(blueprintId, version) {
  const [row] = await db
    .select()
    .from(automationBlueprintVersions)
    .where(and(
      eq(automationBlueprintVersions.blueprintId, blueprintId),
      eq(automationBlueprintVersions.version, version),
    ))
    .limit(1);
  return row || null;
}

export async function getLatest(blueprintId) {
  const parent = await findParent(blueprintId);
  if (!parent) return null;
  const versionRow = await findVersion(blueprintId, parent.currentVersion);
  if (!versionRow) return null;
  return formatBlueprintRow(parent, versionRow);
}

export async function getVersion(blueprintId, version) {
  const parent = await findParent(blueprintId);
  if (!parent) return null;
  const versionRow = await findVersion(blueprintId, version);
  if (!versionRow) return null;
  return formatBlueprintRow(parent, versionRow);
}

export async function getBySession(sessionId) {
  const [row] = await db
    .select({ id: automationBlueprints.id })
    .from(automationBlueprints)
    .where(eq(automationBlueprints.sessionId, sessionId))
    .orderBy(desc(automationBlueprints.updatedAt))
    .limit(1);
  if (!row) return null;
  return getLatest(row.id);
}

export async function saveWorkflow(blueprintId, blueprintVersion, workflow, target = 'n8n') {
  const id = uuidv4();
  await db.insert(blueprintWorkflows).values({
    id,
    blueprintId,
    blueprintVersion,
    target,
    workflowJson: workflow,
  });
  return { id };
}

export async function getLatestWorkflow(blueprintId) {
  const [row] = await db
    .select({ workflowJson: blueprintWorkflows.workflowJson })
    .from(blueprintWorkflows)
    .where(eq(blueprintWorkflows.blueprintId, blueprintId))
    .orderBy(desc(blueprintWorkflows.seq))
    .limit(1);
  if (!row) return null;
  return asJson(row.workflowJson);
}

/**
 * The most recent generated workflow for a Blueprint, WITH the Blueprint version
 * it was generated from, so a caller can detect staleness (workflow generated
 * from v5 while the Blueprint is now v6). Returns null if none generated yet.
 */
export async function getLatestWorkflowRecord(blueprintId) {
  const [row] = await db
    .select({
      workflowJson: blueprintWorkflows.workflowJson,
      blueprintVersion: blueprintWorkflows.blueprintVersion,
      createdAt: blueprintWorkflows.createdAt,
    })
    .from(blueprintWorkflows)
    .where(eq(blueprintWorkflows.blueprintId, blueprintId))
    .orderBy(desc(blueprintWorkflows.seq))
    .limit(1);
  if (!row) return null;
  return {
    workflow: asJson(row.workflowJson),
    generated_from_version: row.blueprintVersion,
    created_at: row.createdAt,
  };
}

/** The workflow generated from a SPECIFIC Blueprint version, or null. */
export async function getWorkflowForBlueprintVersion(blueprintId, blueprintVersion) {
  const [row] = await db
    .select({ workflowJson: blueprintWorkflows.workflowJson })
    .from(blueprintWorkflows)
    .where(and(
      eq(blueprintWorkflows.blueprintId, blueprintId),
      eq(blueprintWorkflows.blueprintVersion, blueprintVersion),
    ))
    .orderBy(desc(blueprintWorkflows.seq))
    .limit(1);
  if (!row) return null;
  return asJson(row.workflowJson);
}

function formatBlueprintRow(parent, versionRow) {
  const readiness = asJson(versionRow.readinessJson);
  return {
    blueprint_id: parent.id,
    session_id: parent.sessionId,
    version: versionRow.version,
    schema_version: versionRow.schemaVersion,
    // The status OF THIS VERSION comes from the version's own readiness snapshot.
    // parent.status is the *current* status and must not be reported for an
    // older version (P0 bug: v1 could report the complete status of v5).
    status: readiness?.status || parent.status,
    // The parent's current version, so callers can tell whether this is the head.
    current_version: parent.currentVersion,
    is_current: versionRow.version === parent.currentVersion,
    blueprint: asJson(versionRow.blueprintJson),
    readiness,
    created_at: versionRow.createdAt,
  };
}
