...
import { retrieveNodeContext, retrieveToolContext } from './retrieval.js';
...
export async function generateN8nWorkflow(bp, { maxRepairs = 2 } = {}) {
  // Retrieve from BOTH Pinecone sources in parallel: n8n node knowledge and the
  // Vyrade tools index (tools + API docs the specialist can wire as HTTP nodes).
  const [nodeCtx, toolCtx] = await Promise.all([
...
Produce the complete, correctly-connected n8n workflow JSON for this Blueprint.`;
  ...
  for (let attempt = 0; attempt <= maxRepairs; attempt++) {
    ...
    if (errors.length === 0) {
      ...
      // Usage rides on the workflow object (non-enumerable-ish meta) so the
      // service can attribute the cost without changing the public shape much.
