...
import { clientIp, UNKNOWN_IP } from './clientIp.js';

// Re-exported so routes/tests have one import for the limiter surface.
export { clientIp, trustProxyHops } from './clientIp.js';
...
export async function enforceRateLimit({ event, email, ip }) {
  ...
  // With no resolvable client IP we deliberately skip the IP bucket rather than
  // lumping every request into one shared 'unknown' counter — that would let a
  // single abuser lock out all users. Email limits still apply. If you see this
  // warning, set TRUST_PROXY (see lib/auth/clientIp.js) so per-IP limits work.
  if (policy.ip && ip && ip !== UNKNOWN_IP) {
...
function warnUnresolvableIp() {
  if (warnedNoIp) return;
  warnedNoIp = true;
  console.warn(
    '[rate-limit] No trusted client IP could be resolved — per-IP limits are INACTIVE ' +
    '(per-email limits still apply). Set TRUST_PROXY=<number of proxies> if this app ' +
    'runs behind a reverse proxy, or deploy behind one that sets a platform IP header.'
  );
}
