...
// Platform lock-in the USER stated as a constraint. This is deliberately NOT
// part of the neutral business process — it records that the user is required
// to stay on / avoid a platform (e.g. "we must stay on n8n"), which the
// downstream specialist must respect. checkNeutrality() exempts this subtree so
// preserving a user constraint doesn't trip the "no implementation terms" rule.
const ImplementationConstraints = z.object({
  required_platforms: z.array(z.string()),   // must be used (contractual/operational)
  prohibited_platforms: z.array(z.string()), // must NOT be used
  existing_platforms: z.array(z.string()),   // already in use at the company
  platform_preferences: z.array(z.string()), // soft preferences
});
...
export function checkNeutrality(bp) {
  // Exempt constraints.implementation_constraints — that subtree is explicitly
  // where a user's platform lock-in (e.g. "required to stay on n8n") is
  // recorded. Everything else must remain platform-neutral.
  const { implementation_constraints, ...restConstraints } = bp.constraints || {};
  const scannable = { ...bp, constraints: restConstraints };

  const text = JSON.stringify(scannable).toLowerCase();
