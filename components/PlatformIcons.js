/**
 * Official platform marks, from Simple Icons (CC0).
 *
 * Every glyph is `currentColor`, so the chip controls it. Two brands invert on
 * dark because their official hex disappears against our dark card:
 *   Make  #6D00CC -> 2.30:1   MCP #000000 -> 1.09:1
 * Inverting (white chip, brand-colored glyph) is how both brands render on dark
 * surfaces, and it keeps the real brand color on screen rather than inventing one.
 *
...
/**
 * hex        chip fill in light mode, glyph fill in dark when inverted
 * invertDark chip becomes white and the glyph takes `hex`
 */
export const PLATFORMS = {
  n8n: { name: 'n8n', hex: '#EA4B71', invertDark: false },
...
export function PlatformChip({ platform, className }) {
  ...
  if (!meta?.hex || !glyph) {
    return (
      <span
        className={cn(
          'flex h-7 w-7 shrink-0 items-center justify-center rounded-md bg-muted text-muted-foreground ring-1 ring-inset ring-border',
          className
        )}
      >
  ...
  return (
    <span
      style={{
        '--chip': meta.hex,
        '--chip-dark': meta.invertDark ? '#FFFFFF' : meta.hex,
        '--glyph': '#FFFFFF',
        '--glyph-dark': meta.invertDark ? meta.hex : '#FFFFFF',
      }}
      className={cn(
        'flex h-7 w-7 shrink-0 items-center justify-center rounded-md ring-1 ring-inset ring-black/10',
        'bg-[var(--chip)] text-[var(--glyph)]',
        'dark:bg-[var(--chip-dark)] dark:text-[var(--glyph-dark)] dark:ring-white/15',
        className
      )}
    >
      {/* h-4/w-4 matches buttonVariants' `[&_svg]:size-4`, so a chip inside a
          Button and a chip inside a plain div render at the same size. */}
